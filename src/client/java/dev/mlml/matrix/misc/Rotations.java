...
public class Rotations {
    ...
    static {
        MatrixMod.eventManager.register(Rotations.class);
    }
    ...
    public static void lookAtPositionSmoothServerSide(Vec3d target, double laziness) {
        double delX = target.x - Objects.requireNonNull(MatrixMod.mc.player).getX();
        double delZ = target.z - MatrixMod.mc.player.getZ();
        double delY = target.y - (MatrixMod.mc.player.getY() + MatrixMod.mc.player.getEyeHeight(MatrixMod.mc.player.getPose()));
    ...
    public static void lookAtPositionSmooth(float pitch, float yaw, float laziness) {
        ...
        delta = MathHelper.wrapDegrees(yaw - MatrixMod.mc.player.getYaw());
        speed = Math.abs(delta / laziness);
        add = speed * (delta >= 0 ? 1 : -1);
        if (add >= 0 && add > delta || add < 0 && add < delta) {
            add = delta;
        }
        MatrixMod.mc.player.setYaw(MatrixMod.mc.player.getYaw() + (float) add);

        delta = MathHelper.wrapDegrees(pitch - MatrixMod.mc.player.getPitch());
        speed = Math.abs(delta / laziness);
        add = speed * (delta >= 0 ? 1 : -1);
        if (add >= 0 && add > delta || add < 0 && add < delta) {
            add = delta;
        }
        MatrixMod.mc.player.setPitch(MatrixMod.mc.player.getPitch() + (float) add);
    }
    ...
    public static void lookAtPositionSmooth(Vec3d target, float laziness) {
        ...
        delta = MathHelper.wrapDegrees(required - MatrixMod.mc.player.getYaw());
        speed = Math.abs(delta / laziness);
        add = speed * (delta >= 0 ? 1 : -1);
        if (add >= 0 && add > delta || add < 0 && add < delta) {
            add = delta;
        ...
        required = degTan;
        delta = MathHelper.wrapDegrees(required - MatrixMod.mc.player.getPitch());
        speed = Math.abs(delta / laziness);
        add = speed * (delta >= 0 ? 1 : -1);
        if (add >= 0 && add > delta || add < 0 && add < delta) {
            add = delta;
    ...
    public static Rotation getPitchYaw(Vec3d targetV3) {
        return getPitchYawFromOtherEntity(Objects.requireNonNull(MatrixMod.mc.player).getEyePos(), targetV3);
    }
    ...
    public static void onPacketReceive(PacketEvent.Received pe) {
         if (pe.getPacket() instanceof PlayerPositionLookS2CPacket p) {
            clientYaw = p.change().yaw();
            clientPitch = p.change().pitch();
        }
    }
}
