...
public class HeadsUpDisplay extends Module {
    private final BooleanSetting showWatermark = config.add(new BooleanSetting("Show Watermark", "Shows the Matrix watermark", true));
    ...
    public final KeyBinding toggleKey = new KeyBinding("key.matrix.hud_toggle", GLFW.GLFW_KEY_LEFT, "category.matrix");
    public final KeyBinding upKey = new KeyBinding("key.matrix.hud_up", GLFW.GLFW_KEY_UP, "category.matrix");
    public final KeyBinding downKey = new KeyBinding("key.matrix.hud_down", GLFW.GLFW_KEY_DOWN, "category.matrix");
    public final KeyBinding settingsKey = new KeyBinding("key.matrix.hud_settings", GLFW.GLFW_KEY_RIGHT, "category.matrix");
    ...
    public HeadsUpDisplay() {
        super("HeadsUpDisplay", "Displays information on your screen", GLFW.GLFW_KEY_UNKNOWN);
    ...
    private void renderText(DrawContext drawContext, String text, Corner corner, int index) {
        int[] base = corner.getBase(MatrixMod.mc.textRenderer.getWidth(text), MatrixMod.mc.textRenderer.fontHeight, index);

        drawContext.drawCenteredTextWithShadow(MatrixMod.mc.textRenderer, text, base[0], base[1], 0xFFFF);
    }
    ...
    public void onTick() {
        ...
        if (settingsKey.wasPressed()) {
            if (isVisible && kcSelected != null) {
                MatrixMod.mc.setScreen(new ConfigScreen(kcSelected));
            }
            kcShowingTicks = 100;
        }
    }
    ...
    public void onRender(DrawContext drawContext, RenderTickCounter tickDelta) {
        ...
        if (!simplify.getValue()) {
            ...
            for (Module m : modules) {
                ...
                width += MatrixMod.mc.textRenderer.getWidth(m.getName() + ", ");

                if (width > MatrixMod.mc.getWindow().getScaledWidth() / 3) {
        ...
        renderText(drawContext, TextFormatter.format("[%2%3Matrix%1 v%2%3%s%1]", TextFormatter.Code.RESET, TextFormatter.Code.BOLD, TextFormatter.Code.GRAY, MatrixMod.VERSION), Corner.TOP_RIGHT, 0);
        count = 1;
        if (showFPS.getValue()) {
            renderText(drawContext, TextFormatter.format("FPS: %2%3%d", TextFormatter.Code.RESET, TextFormatter.Code.BOLD, TextFormatter.Code.GRAY, MatrixMod.mc.getCurrentFps()), Corner.TOP_RIGHT, count++);
        }
        if (showCoordinates.getValue()) {
            renderText(drawContext, TextFormatter.format("X: %2%3%.2f%1 Y: %2%3%.2f%1 Z: %2%3%.2f", TextFormatter.Code.RESET, TextFormatter.Code.BOLD, TextFormatter.Code.GRAY, MatrixMod.mc.player.getX(), MatrixMod.mc.player.getY(), MatrixMod.mc.player.getZ()), Corner.TOP_RIGHT, count++);
        }
        if (showVelocity.getValue()) {
            renderText(drawContext, TextFormatter.format("Vel: %2%3%.2f", TextFormatter.Code.RESET, TextFormatter.Code.BOLD, TextFormatter.Code.GRAY, MatrixMod.mc.player.getVelocity().length()), Corner.TOP_RIGHT, count++);
        }
    }
    ...
    private enum Corner {
        ...
        public int[] getBase(int textWidth, int textHeight, int row) {
            ...
            return switch (this) {
                ...
                case TOP_RIGHT ->
                        new int[]{MatrixMod.mc.getWindow().getScaledWidth() - textWidth / 2 - gap, gap + (textHeight + gap) * row};
                case BOTTOM_LEFT ->
                        new int[]{gap + textWidth / 2, MatrixMod.mc.getWindow().getScaledHeight() + gap + (textHeight + gap) * row};
                case BOTTOM_RIGHT ->
                        new int[]{MatrixMod.mc.getWindow().getScaledWidth() - textWidth / 2 - gap, MatrixMod.mc.getWindow().getScaledHeight() + gap + (textHeight + gap) * row};
            };
        }
    }
}
