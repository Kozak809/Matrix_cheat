...
import dev.mlml.matrix.mixin.IClientPlayerInteractionManagerMixin;
...
public class FastMine extends Module {
    ...
    @Override
    public void onTick() {
        if (MatrixMod.mc.player == null || MatrixMod.mc.world == null || MatrixMod.mc.interactionManager == null) {
            return;
        }

        if (MatrixMod.mc.interactionManager.isBreakingBlock()) {
            BlockPos last = ((IClientPlayerInteractionManagerMixin) MatrixMod.mc.interactionManager).getCurrentBreakingPos();
        ...
        Vec3d p = MatrixMod.mc.gameRenderer.getCamera().getPos();
        if (posList.isEmpty()) {
            return;
        }
        Vec3d latest = posList.get(0);
        if (latest.add(0.5, 0.5, 0.5).distanceTo(p) >= MatrixMod.mc.player.getBlockInteractionRange()) {
            posList.remove(0);
            return;
        }
        BlockPos bp = BlockPos.ofFloored(latest);
        if (Objects.requireNonNull(MatrixMod.mc.world).getBlockState(bp).isAir()) {
            posList.remove(0);
            return;
        }
