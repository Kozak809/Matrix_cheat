...
public class Flight extends Module {
    ...
    private final BooleanSetting keepFlying = config.add(new BooleanSetting("Keep Flying", "Keep flying after touching ground", true));
    ...
    private static PlayerMoveC2SPacket.Full upgrade(PlayerMoveC2SPacket packet) {
        assert MatrixMod.mc.player != null;
        Vec3d pos = MatrixMod.mc.player.getPos();
        return new PlayerMoveC2SPacket.Full(packet.getX(pos.x), packet.getY(pos.y), packet.getZ(pos.z), MatrixMod.mc.player.getYaw(), MatrixMod.mc.player.getPitch(), packet.isOnGround(), MatrixMod.mc.player.horizontalCollision);
    }
    ...
    public void onTick() {
        ...
        switch (mode.getValue()) {
            case Ability:
                MatrixMod.mc.player.getAbilities().setFlySpeed((float) sp);
                MatrixMod.mc.player.getAbilities().allowFlying = true;
                if (keepFlying.getValue()) {
                    MatrixMod.mc.player.getAbilities().flying = true;
                }
                break;
            case Velocity:
                GameOptions options = MatrixMod.mc.options;
                float yaw = MatrixMod.mc.player.getYaw();
                ...
                MatrixMod.mc.player.setVelocity(new Vec3d(nx, ny, nz));
                break;
        }

        if (mode.getValue() != Mode.Ability) {
            MatrixMod.mc.player.getAbilities().allowFlying = wasFlying;
        }
    }
    ...
    public void onEnable() {
        ...
        wasFlying = MatrixMod.mc.player.getAbilities().allowFlying;
    }
    ...
    private PlayerMoveC2SPacket processMovePacket(PlayerMoveC2SPacket packet, Vec3d playerPosition) {
        if (ph_isHoldingModification) {
            ...
            if (System.currentTimeMillis() - ph_lastModificationTime > 1000) {
                ...
                if (!MatrixMod.mc.world.getBlockState(MatrixMod.mc.player.getBlockPos().down()).blocksMovement()) {
